...
- **Congruency check.** Before a doc is "done", verify it's self-consistent and resolvable *by its own reader*: every label / number / `#` / `Q` / phase / path either resolves in-doc, carries a concrete pointer, or is a recognised external ID (a tracker ID is fine; a bare `4b`, `1a`, or `REFERENCES.md` with no anchor is not); sequences count up with no missing siblings (a `4b` implies a `4a`); and the doc's stated scope / length / purpose matches what it actually contains. Cheap to run; catches the errors that quietly confuse a reader or make a doc look sloppy.
